...
| 成员 | 工作 | 贡献比 |
| --- | --- | --- |
| Prurite | CPU 各模块具体功能实现 | 35%|
| NorthernLand1909 | CPU 设计图绘制，测试用例汇编文件编写 | 35%|
| QWQZZZZZ | 代码整理，电子管模块，硬件绑定 | 30%|
...
各子模块主要按时序及功能划分，Multiplexer, Adder 等整合在模块内部。
...
流水线下，ecall 与 hazard 处理的联动十分重要，尤其是因为 ecall 涉及流水线的阻塞及恢复，要理清每个周期的上升沿及下降沿，各模块中应产生/传递哪些控制信号，并在必要时加入 1 周期的偏移（如 Controller 中的 prevDoBranch ）。
